...
{
	private:
	std::vector<std::string>	meta_variables;	//envpp;	// meta variables and header variables and shiznitz
	std::vector<std::string>	uri;			//argv;		// 4.4 The Script Command Line
	...
	public:
	CgiParsing(std::unordered_map<std::string, std::string> headers, char **environ, std::shared_ptr<Request> _request, const std::string &path, const std::string &interpreter); // ServerStruct &serverinfo
	~CgiParsing(void);
	// char	**get_envpp();
...
//                            "SERVER_SOFTWARE" }; // server passed vraiables, envpp
// customizable_variables_names = { "scheme", "protocol-var-name", "extension-var-name"};
// request_variables = "HTTP_"; //HTTP_var, request passed variables, envpp 
/*RULES
