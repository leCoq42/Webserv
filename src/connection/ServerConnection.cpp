...
void ServerConnection::initServerInfo(ServerStruct serverStruct,
                                      ServerInfo &info,
...
void ServerConnection::bindServerSocket(ServerInfo &info) {
  const int reuse = 1;
  if (setsockopt(info.serverFD, SOL_SOCKET, SO_REUSEADDR, &reuse,
                 sizeof(int)) != 0) {
    close(info.serverFD);
    logError("Could not configure socket options: " +
             std::string(
                 std::strerror(errno))); // Fix the error by using std::string()
                                         // around std::strerror(errno)
  }
  if (bind(info.serverFD, (struct sockaddr *)&info.server_addr,
           sizeof(info.server_addr)) == -1) {
    close(info.serverFD);
    logServerError("Failed to bind server socket", info.serverID,
                   info.serverPort);
  }
}

void ServerConnection::listenIncomingConnections(ServerInfo &info) {
  if (listen(info.serverFD, 10) == -1) { // Adjust the value (10) based on the
                                         // performance and scalability needs.
    close(info.serverFD);
    logServerError("Failed to listen for connection on server socket",
                   info.serverID, info.serverPort);
  }
}
